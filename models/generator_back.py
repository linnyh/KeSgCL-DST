...
# ====================================================
        ...
        if syn_feature is not None:
            sequence_output_span = 0.5 * sequence_output + 0.5 * syn_feature
            # todo 门控融合
        ...
        sequence_output_span = selected_score * sequence_output_span  # ? [2, 256, 1024] 句法增强后的序列表征, 用于跨度预测
        sequence_output_cls = selected_score * sequence_output

        sequence_output_span = sample_mm.transpose(-1, -2).bmm( # ? [1,30,256,1024] for span
            sequence_output_span.reshape(sample_mm.shape[:2] + tuple([-1]))).reshape(
            (sample_mm.shape[0], self.n_slot, -1, self.hidden_size))

        sequence_output_cls = sample_mm.transpose(-1, -2).bmm( # ? [1,30,256,1024] for cls
            sequence_output_cls.reshape(sample_mm.shape[:2] + tuple([-1]))).reshape(
            (sample_mm.shape[0], self.n_slot, -1, self.hidden_size))
        ...
        state_pos = state_positions[:, :, :, None].expand(-1, -1, -1, sequence_output_span.size(-1)).long() # ? [1,30,30,1024]
        state_output = torch.gather(sequence_output_span, 2, state_pos) # ? [1,30,30,1024] 
        state_output = state_output.masked_select( # ? [1,30,1,1024]
            torch.eye(self.n_slot).cuda().unsqueeze(0).unsqueeze(-1) == 1).reshape(sequence_output_span.shape[0],
                                                                                   self.n_slot, 1, -1) 
        start_output = self.start_output(sequence_output_span) # ! [1,30, 256, 1024]
        end_output = self.end_output(sequence_output_span)       # ! [1, 30, 256, 1024]
        start_atten_m = state_output.mul(start_output).sum(dim=-1).view(sequence_output_span.shape[0],
                                                                        sequence_output_span.shape[1], -1) / math.sqrt(
            self.hidden_size) # ! [1, 30, 256]
        end_atten_m = state_output.mul(end_output).sum(dim=-1).view(sequence_output_span.shape[0], sequence_output_span.shape[1],
        ...
        # 分类预测
        ques_attn = F.softmax((sequence_output_cls.mul(state_output.view(-1, self.n_slot, 1, self.hidden_size)).sum(
            dim=-1) / math.sqrt(self.hidden_size)).masked_fill(attention_mask.squeeze() == 0, -1e9), dim=-1) # ? [1, 30, 256]
        sequence_pool_output = ques_attn.unsqueeze(-1).mul(sequence_output_cls).sum(dim=-2).squeeze() # ? [30, 1024]
